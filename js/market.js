...
(function () {
  ...
  function tick() {
    ...
    ['gear', 'character'].forEach(function (type) { var rotation = state.data.rotations[type], clock = document.querySelector('[data-market-countdown="' + type + '"]'), note = document.getElementById('market-' + type + '-refresh'), date = apiDate(rotation.ends_at), remaining = date ? Math.max(0, Math.ceil((date.getTime() - (Date.now() + state.serverOffset)) / 1000)) : 0; if (clock) { ensureRotationRing(clock.closest('.market-rotation-clock')); clock.textContent = timeLeft(rotation.ends_at); var host = clock.closest('.market-rotation-clock'); if (host) host.style.setProperty('--market-progress', String(Math.max(0, Math.min(1, 1 - (remaining / WINDOW_SECONDS))))); } if (note) note.textContent = 'Refreshes ' + formatUtc(rotation.ends_at); });
