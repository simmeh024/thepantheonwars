...
- Cache-busting: `css/style.css?v=N` -- bump `N` across all public HTML files plus
  ...
  versions: public v=200, community v=198, and admin v=211. Public pages use
...
- **Interactive Worlds atlas:** `worlds.html` now presents the supplied
  ...
  `js/worlds.js?v=6` maps the existing `worlds.sort_order` values 1–12 to the
  ...
  box. Locked medallions use a saturation-blend + dark SVG circle against the original
  image rather than a second clipped image, so their grayscale state cannot drift. Each
  focus/hover signal also applies a world-specific, restrained gradient to the info
  panel. Retain those rules when revising the atlas. There is no new table or migration.
