...
-- Persistent post-quiz dialogue consequences. See migration_syn_dialogue.sql.
-- Outcomes are unique so a member cannot repeatedly claim a branch reward;
-- flags and fragments are intentionally reusable by future Overlord encounters.
...
CREATE TABLE IF NOT EXISTS mail_templates (
  ...
  registration_age_days INT UNSIGNED NULL,
...
-- Newsletter campaigns are draft-first and snapshot only consented, valid,
-- non-banned recipients at the moment an administrator explicitly starts a
-- send. See sql/migration_mail_campaigns.sql for the production migration.
CREATE TABLE IF NOT EXISTS mail_campaigns (
  ...
  html_body MEDIUMTEXT NOT NULL,
  text_body MEDIUMTEXT NOT NULL,
  status ENUM('draft','ready','sending','sent','paused','failed') NOT NULL DEFAULT 'draft',
  ...
  CONSTRAINT fk_mail_campaign_created_by FOREIGN KEY (created_by) REFERENCES users(id),
  CONSTRAINT fk_mail_campaign_updated_by FOREIGN KEY (updated_by) REFERENCES users(id),
  CONSTRAINT fk_mail_campaign_sent_by FOREIGN KEY (sent_by) REFERENCES users(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS mail_campaign_recipients (
  ...
  status ENUM('pending','accepted','failed','skipped','unsubscribed') NOT NULL DEFAULT 'pending',
  detail VARCHAR(255) NULL,
  sent_at DATETIME NULL,
  UNIQUE KEY uq_campaign_recipient_email (campaign_id, recipient_email),
  KEY idx_campaign_recipient_status (campaign_id, status),
  CONSTRAINT fk_campaign_recipient_campaign FOREIGN KEY (campaign_id) REFERENCES mail_campaigns(id) ON DELETE CASCADE,
  CONSTRAINT fk_campaign_recipient_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- sql/migration_mission_item_levels.sql. Its hidden Field Grade companion is
-- added by sql/migration_mission_field_grade.sql.
...
-- sql/migration_salvage_recovery_contracts.sql adds
-- game_mission_definitions.is_salvage_recovery_contract plus the private
-- game_player_salvage_recovery_contracts ledger. A flagged ordinary mission is
-- sampled only when a player loses a rare-or-better Sweep discovery, and the
...
CREATE TABLE IF NOT EXISTS game_market_rotations (
  id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  offer_type VARCHAR(20) NOT NULL,
  research_refresh_percent DECIMAL(7,2) NOT NULL DEFAULT 0,
  window_started_at DATETIME NOT NULL,
  window_ends_at DATETIME NOT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uq_game_market_rotation_window (offer_type, research_refresh_percent, window_started_at),
  KEY idx_game_market_rotation_active (offer_type, window_ends_at),
  KEY idx_game_market_rotation_scope_active (offer_type, research_refresh_percent, window_ends_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
