<?php
/**
 * Timeline Control image upload endpoint. Mirrors
 * api/admin/known-figures/upload-image.php exactly (multipart/form-data, GD
 * re-encode, server-generated filename, per-folder .htaccess denying PHP
 * execution) -- see api/admin/overlords/upload-image.php for the full
 * security rationale.
 */
...
pw_require_permission('timeline.edit');
...
// uploads/timeline-images/events ships with a committed .htaccess (same as
// uploads/known-figure-images/*) -- not generated here, so the directory is
// never briefly unprotected between creation and first upload.
$uploadDir = __DIR__ . '/../../../uploads/timeline-images/' . $folderName;
if (!is_dir($uploadDir)) {
...
pw_json(['ok' => true, 'url' => '/uploads/timeline-images/' . $folderName . '/' . $filename]);
