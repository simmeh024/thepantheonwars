<?php
/**
 * Overlord Control image upload endpoint. Mirrors
 * api/admin/worlds/upload-image.php exactly (multipart/form-data, GD
...
pw_require_permission('overlords.edit');
...
// uploads/overlord-images/portraits ships with a committed .htaccess (same
// as uploads/world-images/*) -- not generated here, so the directory is
// never briefly unprotected between creation and first upload.
$uploadDir = __DIR__ . '/../../../uploads/overlord-images/' . $folderName;
if (!is_dir($uploadDir)) {
...
pw_json(['ok' => true, 'url' => '/uploads/overlord-images/' . $folderName . '/' . $filename]);
