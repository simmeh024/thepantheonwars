...
try {
    $nodes = $db->query(
        'SELECT n.*, target.name AS target_mission_name, salvage.name AS salvage_name
    ...
    foreach ($nodes as &$node) {
        foreach (['id', 'target_mission_definition_id', 'required_reputation_level', 'credit_cost', 'salvage_loot_definition_id', 'salvage_quantity', 'canvas_x', 'canvas_y', 'sort_order'] as $field) {
    ...
    unset($mission);
    pw_json(['ok' => true, 'nodes' => $nodes, 'salvage' => $salvage, 'missions' => $missions, 'effect_types' => pw_research_effect_types(), 'board' => ['width' => PW_RESEARCH_BOARD_WIDTH, 'height' => PW_RESEARCH_BOARD_HEIGHT]]);
} catch (Throwable $e) {
    pw_error('Could not load Research Management. Confirm that sql/migration_research_system.sql has been run.', 503);
}
