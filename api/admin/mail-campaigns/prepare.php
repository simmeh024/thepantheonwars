...
try {
    ...
    $insert = $db->prepare('INSERT IGNORE INTO mail_campaign_recipients (campaign_id,user_id,recipient_email,recipient_name) SELECT ?,id,email,display_name FROM users WHERE newsletter_subscribed=1 AND email<>"" AND (banned_at IS NULL OR (banned_until IS NOT NULL AND banned_until <= NOW()))');
    $insert->execute([$id]); $count = $db->prepare('SELECT COUNT(*) AS c FROM mail_campaign_recipients WHERE campaign_id = ?'); $count->execute([$id]); $total=(int)$count->fetch()['c'];
