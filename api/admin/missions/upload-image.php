...
$background = imagecolorallocate($destination, 20, 18, 28); imagefilledrectangle($destination, 0, 0, $width, $height, $background);
imagecopyresampled($destination, $source, 0, 0, 0, 0, $width, $height, $sourceWidth, $sourceHeight); imagedestroy($source);
$directory = __DIR__ . '/../../../uploads/mission-crew-images';
if (!is_dir($directory) && !mkdir($directory, 0755, true) && !is_dir($directory)) { imagedestroy($destination); pw_error('Could not prepare the crew image library.'); }
$filename = 'img_' . bin2hex(random_bytes(8)) . '.jpg';
$destinationPath = $directory . '/' . $filename;
if (!imagejpeg($destination, $destinationPath . '.tmp', 85)) { imagedestroy($destination); pw_error('Could not save the processed image.'); }
imagedestroy($destination); rename($destinationPath . '.tmp', $destinationPath);
pw_json(['ok' => true, 'url' => '/uploads/mission-crew-images/' . $filename]);
