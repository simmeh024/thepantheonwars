...
$stmt = $db->prepare(
    'SELECT c.id, c.last_message_at,
            ...
            other.presence_status AS other_presence_status, other.last_active_at AS other_last_active_at,
     ...
     LEFT JOIN direct_conversation_preferences pref ON pref.conversation_id = c.id AND pref.user_id = ?
     LEFT JOIN direct_messages lm ON lm.id = c.last_message_id
     WHERE c.user_low_id = ? OR c.user_high_id = ?
     ORDER BY COALESCE(pref.is_pinned, 0) DESC, c.last_message_at DESC, c.id DESC'
);
...
$conversations = array_map(function ($row) {
    return [
        ...
        'counterpart' => [
            ...
            'presence_status' => pw_public_presence_status($row['other_presence_status'], $row['other_last_active_at']),
