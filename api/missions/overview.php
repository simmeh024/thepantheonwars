...
try {
    ...
    $worldMissions = array_map(static function ($row) use ($claimedCounts, $fatigueReady, $lastCrewByMission, $crew, $db) {
        ...
        /* What this operation's rolls can actually produce. One pooled query
         * for the whole board, filtered per mission in PHP. */
        $row['loot_preview'] = pw_missions_loot_preview($db, (string)$row['world_key'], $row);
    ...
    $publicFields = ['id', 'world_key', 'name', 'slug', 'description', 'mission_type', 'duration_seconds',
        ...
        'watermark_url', 'watermark_opacity', 'fatigue_cost', 'last_crew_ids', 'fit', 'tier_band', 'loot_preview'];
    $slots = [];
